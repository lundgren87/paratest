/** 
 * A simple test of the daxpy form that reproduces a rare coherence 
 * failure on the argodsm/master branch when using a cache_size smaller
 * than argo_size.
 *
 * The test should be run on two ArgoDSM nodes (either hardware or
 * software) without changing argo_size or cache_size in order to 
 * guarantee failure.
 */
	...
	if(nodeid == 0){
		...
		for(i=0; i<arraysize; i++){
			if(out_array[i]!=initvalue*alpha){
				printf("Test failed: out_array[%d]=%.01f (should be %.01f).\n",
